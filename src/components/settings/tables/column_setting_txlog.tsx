...
const column_setting_txlog = (): ColumnDef<any>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="text-left ml-4 text-sm text-gray-600 dark:text-gray-400">{formatted}</div>;
    },
  },
  {
    accessorKey: "account.accountName",
    id: "accountName", // ใช้ตอน Filter
  ...
  // 🔥 ส่วนที่แก้ไข: จัดการตัวเลขแสดงผลให้สมบูรณ์
  {
    accessorKey: "amount",
    ...
    cell: ({ row }) => {
      ...
      // 1. เช็คว่าเป็นลบหรือไม่ (ถ้าเป็นประเภทลดเงิน, โอนออก หรือ ตัวเลขที่ได้มาเป็นลบ)
      const isNegative = type === "ADJUSTMENT_DOWN" || type === "TRANSFER_OUT" || rawAmount < 0;
      
      // 2. แปลงตัวเลขให้เป็นค่าบวกเสมอ เพื่อเอาไปใส่เครื่องหมายเอง

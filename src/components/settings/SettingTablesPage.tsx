...
const SettingTablesPage = ({
  ...
  const onUpdateReservable = async (id: number, isReservable: boolean) => {
    const result = await updateReservableTable(id, isReservable);
    if (result.success) {
      router.refresh();
    }
  };

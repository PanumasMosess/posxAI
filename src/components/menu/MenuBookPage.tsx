...
const MenuImage = memo(
  ({ src, alt, isNearby }: { src: string; alt: string; isNearby: boolean }) => {
        ...
        {!isLoaded && (
          <div className="absolute inset-0 flex items-center justify-center z-0 bg-[#E5E0D8]">
            <UtensilsCrossed
              size={24}
              strokeWidth={1.5}
              className="text-[#A6978C]/30 animate-pulse"
            />
          </div>
        )}
...
const MemoizedMenuCard = memo(
  ({
    ...
    const displayPrice = isPkgChecked
    ...
    const checkboxPosition = isRightPage
      ? "top-3 left-3 sm:top-4 sm:left-4"
      : "top-3 right-3 sm:top-4 sm:right-4";

    const stopNativeClick = (el: HTMLElement | null) => {
      if (el) el.onclick = (e) => e.stopPropagation();
    };
    const pointerPos = useRef({ x: 0, y: 0, time: 0 });
    const handlePointerDown = (e: React.PointerEvent) => {
      pointerPos.current = { x: e.clientX, y: e.clientY, time: Date.now() };
    };
    ...
    return (
      ...
      >
        <div className="absolute inset-0 w-full h-full pointer-events-none">
          {item.img ? (
            ...
            <div className="w-full h-full bg-[#E5E0D8]/60 flex flex-col items-center justify-center text-[#A6978C] opacity-80">
              ...
              />
              <span className="font-serif tracking-widest text-[10px] font-semibold opacity-60 notranslate">
                NO IMAGE
              </span>
            </div>
          )}
        ...
        <div
          className={`absolute ${positionClass} flex shadow-md z-20 max-w-[90%] bg-white/95 rounded-md overflow-hidden pointer-events-none`}
        >
          <div className="bg-[#614D43] text-white px-2.5 sm:px-3 py-1.5 flex items-center justify-center min-w-[36px] sm:min-w-[44px]">
            <span className="font-bold text-[11px] sm:text-[13px] notranslate">
              {item.menuCode || "-"}
            </span>
          </div>
          <div className="text-[#2A2422] px-2.5 sm:px-3 py-1.5 flex flex-col justify-center relative pr-9 sm:pr-10 min-w-[120px]">
            <span className="font-bold text-[11px] sm:text-[12px] leading-tight line-clamp-1">
              {item.menuName}
            </span>
            <span className="font-black text-[#614D43] text-[11px] sm:text-[12px] mt-[1px] notranslate">
        ...
        {item.category?.categoryName === "Entertainer" &&
          item.package_hours > 0 && (
            <div
              ...
              onPointerUp={(e) => {
                e.stopPropagation();
                handlePointerUp(e, () => onTogglePackage(item.id));
              }}
            >
              <Checkbox
                id={`pkg-${item.id}`}
                checked={isPkgChecked}
                className="w-4 h-4 rounded-sm border-[#6B5A4E] data-[state=checked]:bg-[#614D43] pointer-events-none"
              />
              <label className="text-[10px] text-[#2A2422] font-black uppercase pointer-events-none notranslate">
                Pkg {item.package_hours}h
              </label>
            </div>
          )}
      </div>
...
const MenuBookPage = ({
  ...
  const categories = useMemo(() => {
    const sortedDbCategories = relatedData.categories
      .map((c: any) => c.categoryName || c.name)
      .filter(Boolean);
    const activeCatNames = new Set(
      initialItems
        .map((item: any) => item.category?.categoryName)
        .filter(Boolean),
    );
    const catsArray = sortedDbCategories.filter(
      (catName: string) =>
        activeCatNames.has(catName) && catName !== "Entertainer",
    );
  ...
  const categoryIndexPages = useMemo(() => {
    ...
    for (let i = 0; i < displayCats.length; i += catsPerPage)
      chunks.push(displayCats.slice(i, i + catsPerPage));
    return chunks;
  }, [displayCats]);
  ...
  const booksData = useMemo(() => {
    ...
    foodCats.forEach((cat) => {
      const catItems = initialItems.filter(
        (i: any) => i.category?.categoryName === cat,
      );
    ...
    if (mainPages.length % 2 !== 0) {
      ...
      mainPages.push({
        isCover: false,
        title: mainPages[mainPages.length - 1]?.title || "",
        items: [],
        isBlank: true,
      });
      mainPageCatMap[mainPages.length - 1] =
        mainPageCatMap[mainPages.length - 2] || "All";
    ...
    entPages.push({
      isCover: true,
      title: "Entertainer Selection",
      isEntertainerCover: true,
    });
    entPageCatMap[0] = "Entertainer";

    const entItems = initialItems.filter(
      (i: any) => i.category?.categoryName === "Entertainer",
    );
    let j = 0;
    ...
    return {
      main: {
        pages: mainPages,
        catStartMap: mainCatStartMap,
        pageCatMap: mainPageCatMap,
      },
      ent: {
        pages: entPages,
        catStartMap: entCatStartMap,
        pageCatMap: entPageCatMap,
      },
    };
  ...
  const [pendingJumpCategory, setPendingJumpCategory] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (pendingJumpCategory && !isEntertainerMode) {
      const targetPage =
        pendingJumpCategory === "All"
          ? 0
          : booksData.main.catStartMap[pendingJumpCategory];
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      if (filteredCartData.length === 0)
        return toast.warning("ไม่มีรายการในตะกร้า");
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-between items-start pt-4 px-4 sm:px-6 pointer-events-none">
        <div className="pointer-events-auto flex items-center gap-2 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 shadow-lg">
          ...
          <span className="text-white/90 text-[10px] sm:text-xs font-bold tracking-widest uppercase">
            Table{" "}
            <span className="text-orange-500 notranslate">
              {currentTableName}
            </span>
          </span>
        </div>
        ...
        <div className="pointer-events-auto flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-white/80 hover:text-white transition-all bg-black/60 backdrop-blur-md border border-white/10 shadow-lg"
          >
      ...
      <div className="fixed bottom-3 sm:bottom-6 left-0 right-0 z-50 flex justify-center pointer-events-none px-4">
        <div className="pointer-events-auto flex items-center p-1.5 sm:p-2 bg-black/60 backdrop-blur-md rounded-full border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
          <button
            ...
            className="flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-full text-white/80 hover:text-white hover:bg-white/10 transition-all active:scale-95"
          >
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-center">
                            ...
                            <span className="text-stone-400 text-[11px] flex items-center gap-2 mt-0.5">
                              <span className="bg-white/10 px-1.5 py-0.5 rounded text-white/70 notranslate">
                                {item.menuCode || "-"}
                              </span>
          ...
          {activeBookData.pages.map((page, pIndex) => {
            ...
            if (page.isMainCover) {
              return (
                <div
                  key={`page-${pIndex}`}
                  className="bg-[#12100E] w-full h-full relative overflow-hidden flex flex-col items-center justify-center shadow-inner p-4 sm:p-6 border-[12px] sm:border-[16px] border-[#1C1816]"
                >
                  <div
                    className={`absolute top-0 bottom-0 w-16 pointer-events-none z-10 ${isRightPage ? "right-0 bg-gradient-to-l" : "left-0 bg-gradient-to-r"} from-black/80 to-transparent`}
                  />
                  ...
                  <div className="absolute inset-4 sm:inset-6 border border-[#D4AF37]/40 z-20 flex flex-col items-center justify-center p-8">
                    ...
                    <div className="mb-8 text-[#D4AF37] opacity-80">
                      <UtensilsCrossed size={48} strokeWidth={1} />
                    </div>
                    <h1 className="text-5xl sm:text-6xl font-serif text-[#D4AF37] font-bold tracking-[0.25em] text-center mb-6 uppercase drop-shadow-[0_2px_4px_rgba(0,0,0,1)] notranslate">
                      MENU
                    </h1>
                    <div className="w-24 h-[1px] bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent mb-6"></div>
                    <p className="text-[#A38A75] text-[10px] sm:text-[11px] tracking-[0.5em] font-medium uppercase text-center notranslate">
                      Premium Selection
                    </p>
                  </div>
                </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute inset-4 sm:inset-6 border border-[#D4AF37]/20 z-20 flex flex-col items-center justify-center p-8">
                    <div className="mb-6 text-[#D4AF37] opacity-40">
                      <UtensilsCrossed size={40} strokeWidth={1} />
                    </div>
                    <h2 className="text-3xl font-serif text-[#D4AF37] font-bold tracking-[0.2em] mb-4 uppercase drop-shadow-md">
                      Thank You
                    </h2>
                    <p className="text-[#A38A75] text-xs tracking-[0.2em] mb-12">
                      ขอบคุณที่ใช้บริการ
                    </p>
                    <div className="mt-16 w-16 h-[1px] bg-[#D4AF37]/20"></div>
                    <p className="mt-4 text-[9px] text-[#A38A75]/40 tracking-widest uppercase notranslate">
                      END OF MENU
                    </p>
                  </div>
                </div>
            ...
            if (page.isCategoryIndex) {
              return (
                <div
                  key={`page-${pIndex}`}
                  className="bg-[#F5F2EB] w-full h-full relative overflow-hidden flex flex-col shadow-inner p-6 sm:p-10"
                >
                  <div
                    className={`absolute top-0 bottom-0 w-12 pointer-events-none z-10 ${isRightPage ? "right-0 bg-gradient-to-l" : "left-0 bg-gradient-to-r"} from-black/10 to-transparent`}
                  />
                  ...
                  <div
                    className="flex-1 w-full relative z-20 overflow-y-auto pb-6 custom-scrollbar"
                    ...
                    <div className="flex flex-col gap-3 sm:gap-4 px-2">
                      {page.items.map((cat: any, idx: number) => {
                        ...
                        return (
                            ...
                            onPointerUp={(e) =>
                              handlePointerUp(e, () => {
                                if (cat === "Entertainer") {
                                  ...
                                  const targetPage =
                                    booksData.main.catStartMap[cat] || 0;
                          ...
                          >
                            <span
                              className={`pointer-events-none font-bold font-serif tracking-widest uppercase text-[13px] sm:text-sm transition-colors ${isActive ? "text-orange-600" : "text-[#5C4A3D] group-hover:text-orange-600"}`}
                            >
                              {cat === "All" ? "หน้าแรก" : cat}
                            </span>
                            <div
                              className={`pointer-events-none flex-1 border-b-[2px] border-dotted mx-4 relative top-[-6px] transition-colors ${isActive ? "border-orange-600/40" : "border-[#5C4A3D]/20 group-hover:border-orange-600/30"}`}
                            ></div>
                            <ChevronRight
                              size={16}
                              className={`pointer-events-none transition-colors shrink-0 ${isActive ? "text-orange-600" : "text-[#A38A75] group-hover:text-orange-600"}`}
                              strokeWidth={2}
                            />
                          </button>
                        );
            ...
            if (page.isEntertainerCover) {
              return (
                <div
                  key={`page-${pIndex}`}
                  className="bg-[#1A1615] w-full h-full relative overflow-hidden flex flex-col items-center justify-center shadow-inner p-4 border-[12px] border-[#26201E]"
                >
                  <div className="absolute inset-4 border border-[#D4AF37]/30 z-20 flex flex-col items-center justify-center p-8">
                    <div className="mb-4 text-[#D4AF37] opacity-80 text-xl">
                      ✦
                    </div>
                    <h2 className="text-3xl sm:text-4xl text-[#D4AF37] font-serif font-bold text-center uppercase tracking-widest notranslate">
                      {page.title}
                    </h2>
                    <div className="w-12 h-[1px] bg-[#D4AF37]/50 my-4" />
                    <p className="text-[#A38A75] text-[10px] sm:text-[11px] tracking-[0.3em] uppercase text-center mb-8">
                      Exclusive Lounge Mode
                    </p>
                    <button
                      ref={stopNativeClick}
                      style={{ touchAction: "pan-y" }}
            ...
            if (page.isCover && !page.isBlank) {
              return (
                <div
                  key={`page-${pIndex}`}
                  className="bg-[#F5F2EB] w-full h-full relative overflow-hidden flex flex-col items-center justify-center shadow-inner p-4"
                >
                  <div
                    className={`absolute top-0 bottom-0 w-12 pointer-events-none z-10 ${isRightPage ? "right-0 bg-gradient-to-l" : "left-0 bg-gradient-to-r"} from-black/10 to-transparent`}
                  />
                  ...
                  <div className="z-20 flex flex-col items-center justify-center text-center px-8">
                    ...
                    <h2 className="text-4xl sm:text-5xl text-[#5C4A3D] font-serif font-bold leading-tight drop-shadow-sm uppercase tracking-wider">
                      {page.title}
                    </h2>
                    <div className="mt-8 w-24 h-[1px] bg-gradient-to-r from-transparent via-[#5C4A3D] to-transparent" />
                    <span className="text-[#A38A75] text-[10px] tracking-[0.4em] uppercase mt-4 font-semibold notranslate">
                      Signature Menu
                    </span>
                  </div>
                </div>
            ...
            if (page.isBlank) {
              return (
                <div
                  key={`page-${pIndex}`}
                  className="bg-[#EBEBEB] w-full h-full relative shadow-inner"
                >
                  <div
                    className={`absolute top-0 bottom-0 w-8 pointer-events-none z-10 ${isRightPage ? "right-0 bg-gradient-to-l" : "left-0 bg-gradient-to-r"} from-black/10 to-transparent`}
                  />
                </div>
              );
            }
            ...
            return (
              <div
                key={`page-${pIndex}`}
                className="bg-[#EBEBEB] w-full h-full relative overflow-hidden flex flex-col shadow-inner"
              >
                <div
                  className={`absolute top-0 bottom-0 w-8 pointer-events-none z-10 ${isRightPage ? "right-0 bg-gradient-to-l" : "left-0 bg-gradient-to-r"} from-black/10 to-transparent`}
                />
                <div className="flex-1 w-full h-full p-2.5 sm:p-3 pb-8 flex flex-col gap-2.5 sm:gap-3 z-20">
                  {isEntertainerMode ? (
                    <>
                      {itemCount >= 1 && (
                        <div
                          className={`${itemCount === 1 ? "h-full" : "h-1/2"} w-full`}
                        >
                          {renderItemCard(
                            page.items[0],
                            true,
                    ...
                    <>
                      ...
                      {itemCount === 0 && (
                        <div className="h-full w-full flex items-center justify-center">
                          <span className="text-[#333]/20 font-serif tracking-widest text-lg uppercase notranslate">
      ...
      <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
        <SheetContent
          side="left"
          className="w-[300px] bg-[#1a1a1a] border-r border-white/5 p-0 flex flex-col"
        >
          <SheetHeader className="p-6 pb-4 text-left border-b border-white/5">
            <SheetTitle className="text-xl font-serif font-medium text-white/90 tracking-widest uppercase">
              Categories
            </SheetTitle>
          </SheetHeader>
      ...
      <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
        <SheetContent
          side="bottom"
          className="h-[90vh] max-h-[90vh] rounded-t-[2rem] px-0 flex flex-col bg-[#F5F5F5] border-t border-gray-200 text-[#2A2422] overflow-hidden"
        >
          <SheetHeader className="shrink-0 px-6 py-5 border-b border-gray-200 bg-white z-10">
            <SheetTitle className="text-center text-lg font-serif font-bold tracking-[0.1em] text-[#2A2422]">
              YOUR CART ({cartCount})
            </SheetTitle>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
            {filteredCartData.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 py-1">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="font-bold text-sm text-[#2A2422] line-clamp-1">
                            <span className="text-[10px] text-[#614D43] font-sans mr-2 notranslate">
                              {menuItem?.menuCode}
                            </span>
                            {menuItem?.menuName || "Unknown Item"}
                          </h4>
                          <button
                            onClick={() =>
                              handleRemoveFromCart(item.id, item.menuId)
                            }
                            className="text-[#2A2422]/30 hover:text-red-500 p-1 shrink-0"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                        {item.modifiers && item.modifiers.length > 0 && (
                          <div className="mb-2 flex flex-wrap gap-1">
                            {item.modifiers.map((mod: any, index: number) => (
                              <span
                                key={index}
                                className="inline-flex items-center px-2 py-0.5 rounded-sm text-[9px] font-medium bg-gray-100 text-gray-700"
                              >
                                {mod.name}{" "}
                                {mod.price > 0 && (
                                  <span className="ml-1 text-[#614D43] notranslate">
                                    (+{mod.price})
                                  </span>
                                )}
                              </span>
                        ...
                        <div className="flex justify-between items-end mt-1">
                          ...
                          <div className="flex items-center gap-3 bg-gray-50 rounded-full px-2 py-1 border border-gray-200">
                            <button
                              ...
                              className="w-6 h-6 flex items-center justify-center bg-white rounded-full text-[#2A2422] hover:bg-gray-100 transition-colors shadow-sm"
                            >
                              <Minus size={12} />
                            </button>
                            ...
                            <button
                              ...
                              className="w-6 h-6 flex items-center justify-center bg-[#614D43] text-white rounded-full hover:bg-orange-600 transition-colors shadow-sm"
                            >
              ...
              <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-gray-400 gap-3">
                ...
                <p className="tracking-[0.1em] text-xs font-bold">
                  CART IS EMPTY
                </p>
              </div>
            )}
          ...
          <div className="shrink-0 p-5 sm:p-6 border-t border-gray-200 bg-white safe-area-bottom shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-10">
            <div className="flex justify-between items-center mb-5">
              <span className="text-gray-500 text-xs tracking-widest font-bold">
                TOTAL AMOUNT
              </span>
              <span className="text-2xl font-serif font-black text-[#2A2422] notranslate">
